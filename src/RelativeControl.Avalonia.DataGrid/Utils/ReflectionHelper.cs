...
// Copies from Avalonia.Controls.DataGrid.Utils.
namespace RelativeControl.Avalonia.DataGrid.Utils;
...
internal static class TypeHelper {
    ...
    internal static PropertyInfo? GetPropertyOrIndexer(this Type type, string? propertyPath, out object[]? index) {
        ...
        if (elementType != null) {
            ...
            if (typeof(IList<>).MakeGenericType(elementType) is Type genericList && genericList.IsAssignableFrom(type))
    ...
    internal static bool IsNullableEnum(this Type type) {
        return type.IsNullableType() && type.GetGenericArguments().Length == 1 && type.GetGenericArguments()[0].IsEnum;
    }
    ...
    internal static string RemoveDefaultMemberName(string property) {
        if (!string.IsNullOrEmpty(property) && property[property.Length - 1] == RightIndexerToken) {
    ...
    internal static bool GetIsReadOnly(this MemberInfo? memberInfo) {
        if (memberInfo != null) {
            ...
            if (attributes != null && attributes.Length > 0) {
                var readOnlyAttribute = (ReadOnlyAttribute)attributes[0];
