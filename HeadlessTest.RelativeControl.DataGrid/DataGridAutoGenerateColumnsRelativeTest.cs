...
using RelativeControl.Avalonia.DataGrid;
using AvaloniaDataGrid = Avalonia.Controls.DataGrid;

namespace HeadlessTest.RelativeControl.DataGrid;
...
public class DataGridAutoGenerateColumnsRelativeTest {
    private static readonly SampleData[] Source;

    static DataGridAutoGenerateColumnsRelativeTest() {
    ...
    [AvaloniaFact]
    public void Test_RelativeDataGrid_Set_ColumnWidths_Before_Show_When_RelativeSource_Is_Window() {
        AvaloniaDataGrid dataGrid = new() { ItemsSource = Source, AutoGenerateColumns = true };
        RelativeDataGrid.SetIsRelativeEnabled(dataGrid, true);
        Window window = new() { Width = 1440, Content = dataGrid };
        RelativeDataGrid.SetColumnWidths(
            dataGrid,
            string.Join(" ", Enumerable.Range(0, 100).Select(i => $"[{i}]{i / 10D}vw").ToArray()));
        ColumnGenerationInfo? info = RelativeDataGrid.GetColumnGenerationInfo(dataGrid);
        Assert.NotNull(info);
        window.Show();
        Dictionary<int, RelativeLength> expected = Enumerable.Range(0, 100)
                                                             .Select(i => (
                                                                         new RelativeLength(
                                                                             i / 10D,
                                                                             Units.ViewPortWidth,
                                                                             dataGrid), i))
                                                             .ToDictionary(item => item.i, item => item.Item1);
        Assert.All(RelativeDataGrid.GetColumnWidths(dataGrid), v => Assert.Equivalent(expected[v.Key], v.Value));
        info.RelativeApplyStatusChanged += (_, args) => {
            if (args.Property != RelativeDataGrid.ColumnWidthsProperty || args.NewValue != RelativeApplyStatus.Done)
                return;
            Assert.All(
                dataGrid.Columns,
                (col, index) => {
                    RelativeLength width = new(index / 10D, Units.ViewPortWidth, dataGrid);
                    Assert.Equivalent(width, RelativeDataGrid.GetWidth(col));
                    Assert.StrictEqual(width.ActualPixels, col.Width.Value);
                });
        };
    }

    [AvaloniaFact]
    public void Test_RelativeDataGrid_Set_ColumnWidths_Before_Show_When_RelativeSource_Is_Control() {
        AvaloniaDataGrid dataGrid = new() { ItemsSource = Source, AutoGenerateColumns = true };
        RelativeDataGrid.SetIsRelativeEnabled(dataGrid, true);
        Border border = new() { Child = dataGrid };
        Window window = new() { Width = 1440, Content = border };
        RelativeDataGrid.SetColumnWidths(
            dataGrid,
            string.Join(" ", Enumerable.Range(0, 100).Select(i => $"[{i}]{i / 10D}pw").ToArray()));
        ColumnGenerationInfo? info = RelativeDataGrid.GetColumnGenerationInfo(dataGrid);
        Assert.NotNull(info);
        window.Show();
        Dictionary<int, RelativeLength> expected = Enumerable.Range(0, 100)
        ...
        info.RelativeApplyStatusChanged += (_, args) => {
            ...
            Assert.All(
                dataGrid.Columns,
                (col, index) => {
                    RelativeLength width = new(index / 10D, Units.LogicalParentWidth, dataGrid);
                    Assert.Equivalent(width, RelativeDataGrid.GetWidth(col));
                    Assert.StrictEqual(width.ActualPixels, col.Width.Value);
                });
        };
    ...
    [AvaloniaFact]
    public void Test_RelativeDataGrid_Set_ColumnWidths_After_Show_When_RelativeSource_Is_Window() {
        AvaloniaDataGrid dataGrid = new() { ItemsSource = Source, AutoGenerateColumns = true };
        RelativeDataGrid.SetIsRelativeEnabled(dataGrid, true);
        Window window = new() { Width = 1440, Content = dataGrid };
        window.Show();
    ...
    [AvaloniaFact]
    public void Test_RelativeDataGrid_Set_ColumnWidths_After_Show_When_RelativeSource_Is_Control() {
        AvaloniaDataGrid dataGrid = new() { ItemsSource = Source, AutoGenerateColumns = true };
        RelativeDataGrid.SetIsRelativeEnabled(dataGrid, true);
        Border border = new() { Child = dataGrid };
        Window window = new() { Width = 1440, Content = border };
...
public class DataGridRelativeTest {
    [AvaloniaFact]
    public void Test_RelativeDataGrid_Set_Column_Width_Before_Show_When_RelativeSource_Is_Window() {
        var col1 = new DataGridTextColumn();
        var col2 = new DataGridCheckBoxColumn();
        var col3 = new DataGridTemplateColumn();
        AvaloniaDataGrid dataGrid = new() { Columns = { col1, col2, col3 } };
        RelativeDataGrid.SetIsRelativeEnabled(dataGrid, true);
        RelativeDataGrid.SetWidth(col1, "10vw");
        RelativeDataGrid.SetWidth(col2, "20vw");
        RelativeDataGrid.SetWidth(col3, "30vw");
        Window window = new() { Width = 1440, Height = 900, Content = dataGrid };
        window.Show();

        dataGrid.Loaded += (_, _) => {
            Assert.Equivalent(
                new RelativeLength(10, Units.ViewPortWidth, col1, dataGrid),
                RelativeDataGrid.GetWidth(dataGrid.Columns[0]),
                true);

            Assert.Equivalent(
                new RelativeLength(20, Units.ViewPortWidth, col2, dataGrid),
                RelativeDataGrid.GetWidth(dataGrid.Columns[1]),
                true);

            Assert.Equivalent(
                new RelativeLength(30, Units.ViewPortWidth, col3, dataGrid),
                RelativeDataGrid.GetWidth(dataGrid.Columns[2]),
                true);
            Assert.StrictEqual(window.Width * 0.1, dataGrid.Columns[0].Width.Value);
            Assert.StrictEqual(window.Width * 0.2, dataGrid.Columns[1].Width.Value);
            Assert.StrictEqual(window.Width * 0.3, dataGrid.Columns[2].Width.Value);
        };
    }

    [AvaloniaFact]
    public void Test_RelativeDataGrid_Set_Column_Width_After_Show_When_RelativeSource_Is_Window() {
        var col1 = new DataGridTextColumn();
        var col2 = new DataGridCheckBoxColumn();
        var col3 = new DataGridTemplateColumn();
        AvaloniaDataGrid dataGrid = new() { Columns = { col1, col2, col3 } };
        ...
        dataGrid.Loaded += (_, _) => {
            ...
            Assert.Equivalent(
                ...
                RelativeDataGrid.GetWidth(dataGrid.Columns[2]),
                true);
            Assert.StrictEqual(1440 * 0.1, dataGrid.Columns[0].Width.Value);
            Assert.StrictEqual(1440 * 0.2, dataGrid.Columns[1].Width.Value);
            Assert.StrictEqual(1440 * 0.3, dataGrid.Columns[2].Width.Value);
        };
    }

    [AvaloniaFact]
    public void Test_RelativeDataGrid_Set_Column_Width_Before_Show_When_RelativeSource_Is_Control() {
        var col1 = new DataGridTextColumn();
        var col2 = new DataGridCheckBoxColumn();
        var col3 = new DataGridTemplateColumn();
        AvaloniaDataGrid dataGrid = new() { Columns = { col1, col2, col3 } };
        RelativeDataGrid.SetIsRelativeEnabled(dataGrid, true);
        RelativeDataGrid.SetWidth(col1, "10pw");
        RelativeDataGrid.SetWidth(col2, "20pw");
        RelativeDataGrid.SetWidth(col3, "30pw");
        Border border = new() { Width = 1440, Height = 900, Child = dataGrid };
        Window window = new() { Width = 1920, Height = 1080, Content = border };
        window.Show();
        dataGrid.Loaded += (_, _) => {
            Assert.Equivalent(
                new RelativeLength(10, Units.LogicalParentWidth, col1, dataGrid),
                RelativeDataGrid.GetWidth(dataGrid.Columns[0]),
                true);

            Assert.Equivalent(
                new RelativeLength(20, Units.LogicalParentWidth, col2, dataGrid),
                RelativeDataGrid.GetWidth(dataGrid.Columns[1]),
                true);

            Assert.Equivalent(
                new RelativeLength(30, Units.LogicalParentWidth, col3, dataGrid),
                RelativeDataGrid.GetWidth(dataGrid.Columns[2]),
                true);
            Assert.StrictEqual(1440 * 0.1, dataGrid.Columns[0].Width.Value);
            Assert.StrictEqual(1440 * 0.2, dataGrid.Columns[1].Width.Value);
            ...
            border.Width = 800;
            border.Height = 600;
            Assert.StrictEqual(800 * 0.1, dataGrid.Columns[0].Width.Value);
            Assert.StrictEqual(800 * 0.2, dataGrid.Columns[1].Width.Value);
            Assert.StrictEqual(800 * 0.3, dataGrid.Columns[2].Width.Value);
        };
    }

    [AvaloniaFact]
    public void Test_RelativeDataGrid_Set_Column_Width_After_Show_When_RelativeSource_Is_Control() {
        var col1 = new DataGridTextColumn();
        var col2 = new DataGridCheckBoxColumn();
        var col3 = new DataGridTemplateColumn();
        AvaloniaDataGrid dataGrid = new() { Columns = { col1, col2, col3 } };
    ...
    [AvaloniaFact]
    public void Test_RelativeDataGrid_Set_Column_Width_After_Loaded() {
        var col1 = new DataGridTextColumn();
        var col2 = new DataGridCheckBoxColumn();
        var col3 = new DataGridTemplateColumn();
        AvaloniaDataGrid dataGrid = new() { Columns = { col1, col2, col3 } };
